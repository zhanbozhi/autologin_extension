...
background.checkLoginStatus(setupLoggedinUI, setupUnloggedinUI);
...
function setupUnloggedinUI() {
    ...
    addDiv.classList.add('nav');
    var addButton = document.createElement('img');
    addButton.classList.add('add-button');
    addButton.setAttribute('alt', '添加');
    addButton.setAttribute('src', browser.extension.getURL('src/img/add_button.png'));
    addButton.onclick = () => {
        add();
    }
    ...
    gettingAllStorageItems.then(results => {
        ...
        for (let i = 0; i < keys.length; i ++) {
            ...
            moreButton.setAttribute('src', browser.extension.getURL('src/img/more_button.png'));
...
function editOrAdd(account, edit) {
    ...
    backButton.setAttribute('src', browser.extension.getURL('src/img/back_button.png'));
    backButton.onclick = () => {
       removeEditPage();
       showAccountsPage();
    }
